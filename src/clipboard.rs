//! File clipboard using `x-special/gnome-copied-files` plus `text/uri-list`.

use crate::gtk::prelude::*;
use crate::{gdk, gio, glib};

const FILES_MIME: &str = "x-special/gnome-copied-files";
...
pub fn set(clipboard: &gdk::Clipboard, files: &[gio::File], cut: bool) {
    ...
    let provider = gdk::ContentProvider::new_union(&[
        gdk::ContentProvider::for_bytes(FILES_MIME, &glib::Bytes::from_owned(text.into_bytes())),
...
pub fn has_files(clipboard: &gdk::Clipboard) -> bool {
    ...
    f.contain_mime_type(FILES_MIME) || f.contain_mime_type(URI_LIST)
}
...
pub async fn read(clipboard: &gdk::Clipboard) -> Option<(Vec<gio::File>, bool)> {
    let (stream, mime) = clipboard
        .read_future(&[FILES_MIME, URI_LIST], glib::Priority::DEFAULT)
        .await
        .ok()?;
