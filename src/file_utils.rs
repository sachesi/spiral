...
/// A share whose name ends in `$` is one SMB servers keep out of their lists: `print$` with the
...
mod tests {
    ...
    /// The shares SMB servers keep out of their lists, the ones ending in `$`, are hidden here
