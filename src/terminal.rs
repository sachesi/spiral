...
/// start those itself -- it will not go looking for a terminal emulator -- so an editor that runs
/// in a terminal, set as the default for a file type, would only ever say it could not be
