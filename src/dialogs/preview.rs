...
fn image_size(path: &Path) -> Option<(i32, i32)> {
    let (width, height, jpeg) = match head_of(path, IMAGE_HEAD).and_then(|h| header_size(&h)) {
        Some(size) => size,
        None => {
            let (format, width, height) = gtk::gdk_pixbuf::Pixbuf::file_info(path)?;
            (width, height, format.name().as_deref() == Some("jpeg"))
        }
    };
    if width <= 0 || height <= 0 {
        return None;
...
fn video_size(path: &Path) -> Option<(f64, f64)> {
...
/// the clusters.
const MATROSKA_SCAN: usize = 2 * 1024 * 1024;
...
fn mp4_size<R: std::io::Read + std::io::Seek>(file: &mut R) -> Option<(f64, f64)> {
...
fn moov_size(moov: &[u8]) -> Option<(f64, f64)> {
...
fn tkhd_size(body: &[u8]) -> Option<(f64, f64)> {
...
fn matroska_size(data: &[u8]) -> Option<(f64, f64)> {
